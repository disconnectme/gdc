		...
		if(reqOrig != null && reqOrig.host!="browser" && contLoc.host!="browser" && contLoc.host!="global"){
			...
			if( reqOrig.host !=contLoc.host && !this.isMatching(reqOrig.host, this.DOMAINS) && this.isMatching(contLoc.host, this.DOMAINS) && typeof aContext.ownerDocument != null){
				...
				try{
					...
					if(typeof aContext.ownerDocument.GgdcCount == "undefined"){
						aContext.ownerDocument.GgdcCount = 1;
					}
					else{
						aContext.ownerDocument.GgdcCount += 1;						
					}					
					
					var finalBlocking = aContext.ownerDocument.defaultView.content.localStorage.getItem('GgdcStatus');
...
if (XPCOMUtils.generateNSGetFactory)
	var NSGetFactory = XPCOMUtils.generateNSGetFactory([GgdcContentPolicy]);
else
	var NSGetModule = XPCOMUtils.generateNSGetModule([GgdcContentPolicy]);
